...
if __name__ == '__main__':
    ...
    parser.add_argument('--headers', nargs='*',
                        help='HTTP headers as key-value pairs')
    parser.add_argument('--params', nargs='*',
                        help='HTTP query parameters as key-value pairs')
    parser.add_argument('--data', nargs='*',
                        help='HTTP request body data as key-value pairs')

    args = parser.parse_args()

    headers = dict(pair.split('=')
                   for pair in args.headers) if args.headers else None
    params = dict(pair.split('=')
                  for pair in args.params) if args.params else None
